...
export default function Home() {
  const handleNavigation = (path: string) => {
    // Aqui você pode adicionar lógica de autenticação se precisar
    console.log(`Indo para: ${process.env.NEXT_PUBLIC_API_BASE}${path}`);
    window.location.href = path;
  };

  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24 bg-black text-white">
      <h1 className="text-6xl font-bold mb-12 text-red-600">CABAPUÃ CONNECT</h1>
      
      <div className="grid grid-cols-1 gap-6 w-full max-w-md">
        <button 
          onClick={() => handleNavigation('/academia')}
          className="p-6 bg-red-700 hover:bg-red-800 rounded-lg text-2xl font-bold transition"
        >
          Academia
        </button>
        <button 
          onClick={() => handleNavigation('/clinica')}
          className="p-6 bg-purple-700 hover:bg-purple-800 rounded-lg text-2xl font-bold transition"
        >
          Clínica Selene
        </button>
        <button 
          onClick={() => handleNavigation('/loja')}
          className="p-6 bg-yellow-600 hover:bg-yellow-700 rounded-lg text-2xl font-bold transition"
        >
          Loja Uracan
        </button>
      </div>
  ...
  return (
    ...
    >
      ...
      <header className="w-full max-w-md px-6 pt-10 pb-4 text-center animate-fade-up">
        ...
        >
          Bem-vindo ao <span className="text-[#b40000]">Cabapuã</span>
        </h1>
